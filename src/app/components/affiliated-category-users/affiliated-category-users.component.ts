...
export class AffiliatedCategoryUsersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this._constant.showAffCourseWithoutCategory == false) {
      ...
      if (this._constant.filteredCoursesList.length > 0) {
        ...
        if(this.router.url === 'affiliated-catogory-users'){
          this.subscription = this.authService.on('downloadInvoice').subscribe(() => this.downloadInvoiceAffCat());
        }
      }
  ...
  updateDoctorStatusListFromApi(api_res): void {
    ...
    for (let key in this.doctorListNewArr) {
      ...
      */
      if (this.doctorListNewArr[key]['status'] != 'Online') { //  || this.doctorListNewArr[key]['status'] == 'Busy' || this.doctorListNewArr[key]['status'] == null){
        ...
        // console.log(api_res)
        let _api_date: any = new Date(api_res['timestamp']);
        let cur_date: any = new Date();
    ...
    this.doctorList.forEach((doctor) => {
      if (!uniqueDoctorList.some((uniqueDoctor) => uniqueDoctor.ihl_consultant_id === doctor.ihl_consultant_id)) {
        uniqueDoctorList.push(doctor);
      }
    });
    this.doctorList = uniqueDoctorList;
